...
//DetectorConstruction::DetectorConstruction(G4double gap_input, G4double NPS_distance, G4double NPS_angle)
//20181026(start)
//DetectorConstruction::DetectorConstruction(G4double gap_input, G4double NPS_distance, G4double NPS_angle, G4bool field_input)
//20190409(start)
// DetectorConstruction::DetectorConstruction(G4double gap_input, G4double NPS_distance, G4double NPS_angle, G4bool field_input, G4double field_str)
  ...
  fDetectorMessenger(0)
{
  ...
  fChamberOuterRadius = 22.5*inch,
    ...
    //20180328(start)
    //  fChamberInnerRadius = 0.5*41*inch,
    fChamberInnerRadius = 20.5*inch,
    //20180328(start)
    //fChamberHight       = 2988*mm;
    fChamberHight       = 24.25*2*inch;//20180313 to be modified
  //20180313(finish)
  //20171122(finish)
  //20171124(start)
  ...
  fWindowFrameThickness = 1.25*inch;//20130313 guessed
  fWindowFrameHight = 20*inch;//20130313 guessed
  ...
  //20180313(start)
  //20180329(start)
  // fWindowInnerJoint1InnerRadius   = 0.5*1.625*inch;
  // fWindowInnerJoint1OuterRadius   = 0.5*2.5*inch;
  // fWindowInnerJoint1Thickness     = 0.5*inch;
  // fWindowInnerJoint2InnerRadius   = 0.5*1.7*inch;
  // fWindowInnerJoint2OuterRadius   = 0.5*2.38*inch;
  // fWindowInnerJoint2Thickness     = 0.109*inch;
  // fWindowOuterJoint1InnerRadius   = 0.5*1.7*inch;//20180314 guessed
  // fWindowOuterJoint1OuterRadius   = 0.5*2.38*inch;//20180314 guessed
  // fWindowOuterJoint1Thickness     = 0.016*inch;//20180314 guessed
  // fWindowOuterJoint2InnerRadius   = 0.5*1.46*inch;
  // fWindowOuterJoint2OuterRadius   = 0.5*1.69*inch;
  // fWindowOuterJoint2Thickness     = 2.0625*inch;
  // fWindowOuterJoint2_1InnerRadius = fWindowOuterJoint2OuterRadius;
  // fWindowOuterJoint2_1OuterRadius = 0.28*inch + fWindowOuterJoint2OuterRadius;
  // fWindowOuterJoint2_1Thickness   = 0.19*inch;
  // fWindowOuterJoint2_1Position    = 0.62*inch;
  // fWindowOuterJoint3InnerRadius   = 0.5*1.457*inch;//20180314 guessed
  // fWindowOuterJoint3OuterRadius   = 0.5*1.693*inch;//20180314 guessed
  // fWindowOuterJoint3Thickness     = 2.2875*inch;//20180314 guessed
  // fWindowOuterJoint3_1InnerRadius = fWindowOuterJoint3OuterRadius;//20180314 guessed
  // fWindowOuterJoint3_1OuterRadius = 2.75*inch ;//20180314 guessed
  // fWindowOuterJoint3_1Thickness   = 0.68*inch;//20180314 guessed
  ...
  //20180328(start)
  //20180329(start)
  // fBeampipe1OuterRadius = 0.5*2.375*inch;
  // fBeampipe1InnerRadius = fBeampipe1OuterRadius - 0.154*inch;
  // fBeampipe1Length      = 88.75*inch;
  // fBeampipe2OuterRadius = 0.5*3.50*inch;
  // fBeampipe2InnerRadius = fBeampipe2OuterRadius - 0.216*inch;
  // fBeampipe2Length      = 103.66*inch;
  ...
  fBeampipe2Length      = 129.633*inch;//20180417 changed from 2620.138*mm;<-this is the actual length of the pipe.
  ...
  */  
  ...
  fMom_pos_Y = 0*mm;
  //fMom_pos_Z = 4000*mm - 0.5*fMom_Z + 0.5*fCrystal_Z;// 0.5*200.5*mm;//positioning the NPS front face at 4m.
  //20180209(start)
  //  fMom_pos_Z = NPS_distance*mm + 0.5*fSingle_Z;//the position of front face wrapper of the crystal is 4m away from the center of the world. But! the fSingle_Z is not defined yet! You need to delcare it once again after fSingle_Z is valued
  //20180209(finish)

  //20171121(start)
  //Angle in radian
  fMom_theta = -NPS_angle;//[rad] //20180417 -12*pi/180.;//20180417 changed from -8.93*pi/180.;//-15.*pi/180.;//-8.93*pi/180.;//-7.93*pi/180.;
  //20171121(finish)

  //  fTemp_X = 1355*mm;//temp control box(mother volume#2)(contains detectors)
  ...
  /*
    ...
    fBulk_X;//Singles combined into one column(total 30 columns)(mother volume#4)
  ...
  */
  ...
  //20171121(start)
  //size of the physical sweeping magent
  //20180404(start)
  // fSweepingMagnet_X = 40.97*cm;
  // fSweepingMagnet_Y = 60.*cm;//I do not know the Sweeping Magnets hight yet.
  // fSweepingMagnet_Z = 100.*cm;
  // fSweepingMagnet_pos_Z = 157.*cm;
  // fSweepingMagnet_theta = -2.70*pi/180.;
  fSweepingMagnet_X = 55.925*inch;
  fSweepingMagnet_Y = 60*inch;
  fSweepingMagnet_Z = 61.1*inch;//20181218, changed from 61.2*inch to 61.1*inch//61.2*inch;
  //20181016(start)
  // //20180412(start)
  // fSweepingMagnet_pos_X = 15.*cm;
  // fSweepingMagnet_pos_Y = 0.*cm;
  // //20180412(finish)
  // fSweepingMagnet_pos_Z = 157.*cm;
  //20181016(finish)
  //21080417(start)//STP : step file for such as CAD
  //20190902(start)
  //defined below...not here
  // fSweepingMagnetSTPcenter_X = 17.3763*inch;
  // fSweepingMagnetSTPcenter_Y = 0*inch;
  // fSweepingMagnetSTPcenter_Z = 55.877*inch;
  //20190905(start)
  // fSweepingMagnetSTPcenter_X = (8.0375*cos(2.*pi/180.) + 14.15*sin(2.*pi/180.) + 1.25928)*inch;
  // fSweepingMagnetSTPcenter_Y = 0*inch;
  // fSweepingMagnetSTPcenter_Z = (43.2245 + 14.15*cos(2.*pi/180.) - 8.0375*sin(2.*pi/180.))*inch;
  //20190905(finish)
  //20190902(finish)

  //20190905(start)
  //20200727(start)
  // fSweepingMagnet_angle = 4.*TMath::DegToRad();//magnet_center's angle with respect to the NPS: 4deg(default) or 5.5deg
  ...
  //20190902(start)
  // fSweepingMagnetSTPshift_X = /*-0.5*fSweepingMagnet_X +*/ ((14.4 - 0.5*43.8/*part1 whole size*/ - 0.5*15/*part1_3 x size*/ - 11.6448)*inch + 0.5*fSweepingMagnet_X);
  // fSweepingMagnetSTPshift_Y = 0*inch;
  // fSweepingMagnetSTPshift_Z = /*-0.5*fSweepingMagnet_Z +*/ ((-0.5*39.5/*part1_3 z size*/ - 5.2/*part6_3 z size*/ - 0.5*4/*part4_2 z size*/ - (0.5*39.5/*part1_3 z size*/ + 5.2/*part6_3 z size*/))*inch + 0.5*4*inch/*part4_2_z_size*/ + 20.9844*inch + 0.5*fSweepingMagnet_Z);
  //20190905(start)
  // fSweepingMagnetSTPshift_X = 0.;
  // fSweepingMagnetSTPshift_Y = 0*inch;
  // fSweepingMagnetSTPshift_Z = 0.;
  ...
  //20181016(start)
  //20190905(start)
  // fSweepingMagnet_pos = sqrt( (fSweepingMagnetSTPcenter_X - fSweepingMagnetSTPshift_X)*(fSweepingMagnetSTPcenter_X - fSweepingMagnetSTPshift_X) + (fSweepingMagnetSTPcenter_Y - fSweepingMagnetSTPshift_Y)*(fSweepingMagnetSTPcenter_Y - fSweepingMagnetSTPshift_Y) + (fSweepingMagnetSTPcenter_Z - fSweepingMagnetSTPshift_Z)*(fSweepingMagnetSTPcenter_Z - fSweepingMagnetSTPshift_Z) );
  ...
  //20181016(start)
  //20190624(start)
  //NPS_angle is radian already.
  // fSweepingMagnet_arm_theta = -NPS_angle*pi/180. - ( asin( (fSweepingMagnetSTPcenter_X - fSweepingMagnetSTPshift_X)/(fSweepingMagnet_pos) ) -  6.3*pi/180. );
  //20190902(start)
  //This is the global angle of the magnet. If magnet is tilted, use fSweepingMagnet_theta(currently no tilting)
  // fSweepingMagnet_arm_theta = -NPS_angle - ( asin( (fSweepingMagnetSTPcenter_X - fSweepingMagnetSTPshift_X)/(fSweepingMagnet_pos) ) -  6.3*pi/180. );
  // fSweepingMagnet_arm_theta = -NPS_angle + 4*pi/180.;
  //20190903(start)
  //20190905(start)
  // fSweepingMagnet_arm_theta = -NPS_angle - TMath::ATan(fSweepingMagnetSTPcenter_X/fSweepingMagnetSTPcenter_Z) + 6.*TMath::DegToRad();//When NPS is 6 deg.
  ...
  //20190905(finish)
  // fSweepingMagnet_arm_theta = -NPS_angle - TMath::ATan(fSweepingMagnetSTPcenter_X/fSweepingMagnetSTPcenter_Z) + 6.*TMath::DegToRad() + 1.5*pi/180.;//some settings has 5.5 degree difference between magnet and NPS, which is a default. When close to the beam-pipe: 4 degrees. That is why I am adding 1.5 degree (based on the fSweepingMagnet_theta)
  //20190903(finish)
  //20190902(finish)
  //20190624(finish)
  //20181016(finish) 
  ...
  //20180928(start)
  //  fSweepingMagnet_theta = -8.27*pi/180.;//20180417 changed from -2.2*pi/180.;//20180412 changed from -2.70*pi/180.
  //Angle in radian
  //20190624(start)
  //NPS_anle is radian already
  // fSweepingMagnet_theta = -NPS_angle*pi/180. -1.97*pi/180.;//to make -8.27degrees when NPS_angle is -6.3degrees. Sweeping Magnet moves along with NPS.
  // fSweepingMagnetField_theta = NPS_angle*pi/180. - 4.1*pi/180.;//to make 2.2degrees when NPS_angle is 6.3degrees. 
  //20190902(start)
  //based on https://userweb.jlab.org/~medeiros/NPS/DETECTOR/STEP-detector_reference.stp//NPS is positioned 6 degrees.
  //20190903(start)
  //20190905(start)
  // fSweepingMagnet_theta = -NPS_angle + 4*pi/180.;//to make -2degrees when NPS_angle is -6degrees. Sweeping Magnet moves along with NPS.
  ...
  //20190905(finish)
  // fSweepingMagnet_theta = -NPS_angle + 4*pi/180. + 1.5*pi/180.;//some settings has 5.5 degree difference between magnet and NPS, which is a default. When close to the beam-pipe: 4 degrees.
  //20190903(finish)
  //20190902(finish)
  //20190903(start)
  //20190905(start)
  // fSweepingMagnetField_theta = NPS_angle - 4.1*pi/180.;//to make 2.2degrees when NPS_angle is 6.3degrees. 
  ...
  //20190905(finish)
  // fSweepingMagnetField_theta = NPS_angle - 4.1*pi/180. - 1.5*pi/180.;//some settings has 5.5 degree difference between magnet and NPS, which is a default. When close to the beam-pipe: 4 degrees.
  //20190903(finish)
  //20190624(finish)
  //20180928(finish)
  //20180404(finish)
  ...
  fSweepingMagnet_Mom_X = 80.*cm + 1.*cm;
  fSweepingMagnet_Mom_Y = 80.*cm + 1.*cm;
  fSweepingMagnet_Mom_Z = 140.*cm + 1.*cm;
  //20171130(finish)
...
void DetectorConstruction::DefineMaterials()
{
  ...
  fWrapMater->AddElement(C, fractionmass=10./22.);//it should be mass fraction!! modify it!!
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  //20180209(start)
  fMom_pos_Z = fNPS_distance*mm + 0.5*fSingle_Z;//the position of front face wrapper of the crystal is 4m away from the center of the world.
  //20180209(finish)
  ...
  // G4double fWindowStartTheta = (90.+180.)*pi/180.;//window beginning position
  // G4double fWindowDeltaTheta = 133.*pi/180.;//window size 127deg
  // G4double fWindowFrameStartTheta =  fWindowStartTheta;//to match the center of the window
  // G4double fWindowFrameDeltaTheta = fWindowDeltaTheta;//total 133 deg.
  //20180328(finish)
  ...
  //20171123(finish)
  new G4PVPlacement(xTargetRot,//20181218, back to inside the chamber//0,//xTargetRot,//20171123
  		    G4ThreeVector(0.*mm, 0.*mm, 0.*mm /* 0.5*fTargetCoverLength */),//G4ThreeVector(0.*mm, -0.5*fTargetCoverLength, 0.*mm),//20171123//
  		    fLogicTargetCover,            //its logical volume
  		    "TargetCover_pos",               //its name
  		    fLogicInnerChamber,//20181218, back to inside the chamber//lWorld,//fLogicInnerChamber,//20171123 put it inside the scattering chamber//lWorld,                     //its mother  volume
  		    false,                 //no boolean operation
  		    0,                     //copy number
  		    fCheckOverlaps);       // checking overlaps 
  ...
  new G4PVPlacement(xTargetRot,//20181218, back to inside the chamber//0,//xTargetRot,//20171123
  		    G4ThreeVector(0.*mm, -0.5*fTargetCoverLength, 0.*mm ),//20181218, back to inside the chamber//,G4ThreeVector(0.*mm, 0.*mm, 0.5*fTargetCoverLength /* fTargetCoverLength */),//G4ThreeVector(0.*mm, -fTargetCoverLength, 0.*mm ),//20171123//G4ThreeVector(0.*mm, 0.*mm, fTargetCoverLength),
  ...
  new G4PVPlacement(0,                     //no rotation
		    G4ThreeVector(0.*mm, 0.*mm, 0.5*fTargetWindowThickness),//20181218, back to inside the chamber//G4ThreeVector(0.*mm, 0.*mm, 0.*mm /* 0.5*fTargetWindowThickness */),//G4ThreeVector(0.*mm, -0.5*fTargetWindowThickness, 0.*mm),//20171123//G4ThreeVector(0.*mm, 0.*mm, 0.5*fTargetWindowThickness),
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness - fWindowThickness/*20180403*/ -fWindowInnerJoint2Thickness - 0.5*fWindowInnerJoint1Thickness),
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness - fWindowThickness/*20180403*/ - 0.5*fWindowInnerJoint2Thickness),
  ...
  G4Tubs*
    sWindowOuterJoint2 = new G4Tubs("WindowOuterJoint2_sol",
				    0.,//fWindowOuterJoint2InnerRadius,//inner radius//will be filled with vacuum
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness*/ +0.5*fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position),
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* ????20180403 */ /*+ fWindowOuterJoint1Thickness */ + 0.5*fWindowOuterJoint2_1Thickness),
  ...
  new G4PVPlacement(zPipeRot,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*//*+ fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + 0.5*fWindowOuterJoint2_2dz),
  ...
  G4Tubs*
    sWindowOuterJoint2_2Vacuum = new G4Tubs("WindowOuterJoint2_2Vacuum_sol",
					    0.,//fWindowOuterJoint2_2InnerRadius,//inner radius//will be filled with vacuum
					    fWindowOuterJoint2InnerRadius,//outer radius
  ...
  G4Tubs*
    sWindowOuterJoint2_3 = new G4Tubs("WindowOuterJoint2_3_sol",
				      0.,//fWindowOuterJoint2_3InnerRadius,//inner radius//will be filled with vacuum
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /*+ fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz + 0.5*fWindowOuterJoint2_3Thickness),
  ...
  // new G4PVPlacement(0,
  // 		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ + fWindowOuterJoint1Thickness + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint3Thickness - 0.5*fWindowOuterJoint3_1Thickness),
  ...
  //20180329(start)
  // G4Tubs*
  //   sBeampipe1 = new G4Tubs("Beampipe_sol",
  // 			    0.,//fWindowOuterJoint3InnerRadius,//inner radius//will be filled with vacuum
  // 			    fBeampipe1OuterRadius,//outer radius
  // 			    0.5*fBeampipe1Length,//hight. lenght
  // 			    0.,//starting angle
  // 			    twopi);//ending angle
  ...
  new G4PVPlacement(zPipeRot,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /*+ fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz /* + fWindowOuterJoint2_3Thickness*/  + 0.5*fBeampipe1Length),
  		    fLogicBeampipe1,
  		    "Beampipe1_pos",
  		    lWorld,
  		    false,
  ...
  //20180329(start)
  // G4Tubs*
  //   sBeampipe1Vacuum = new G4Tubs("Beampipe1Vacuum_sol",
  // 				  0.,//fWindowOuterJoint3InnerRadius,//inner radius//will be filled with vacuum
  // 				  fBeampipe1InnerRadius,//outer radius
  // 				  0.5*fBeampipe1Length,//hight. lenght
  // 				  0.,//starting angle
  // 				  twopi);//ending angle
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz /* + fWindowOuterJoint2_3Thickness */ + fBeampipe1Length + 0.5*fBeampipe2Length),
  		    fLogicBeampipe2,
  		    "Beampipe2_pos",
  		    lWorld,
  		    false,
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz /* + fWindowOuterJoint2_3Thickness */ + fBeampipe1Length - 0.5*fBeampipe2FrontCoverThickness),
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz /* + fWindowOuterJoint2_3Thickness */ + fBeampipe1Length + fBeampipe2Length + 0.5*fBeampipe3Length),
  		    fLogicBeampipe3,
  		    "Beampipe3_pos",
  		    lWorld,
  		    false,
  ...
  //
  // G4Tubs*
  //   sBeampipe3FrontCover_before_sub = new G4Tubs("Beampipe3FrontCover_before_sub_sol",
  // 						 0.,//inner radius
  // 						 fBeampipe3OuterRadius,//outer radius
  ...
  // G4SubtractionSolid* sBeampipe3FrontCover = new G4SubtractionSolid("Beampipe3FrontCover_sol",
  // 								    sBeampipe3FrontCover_before_sub,
  // 								    sBeampipe2,
  // 								    zPipeRot,
  // 								    G4ThreeVector(0, 0, -0.5*fBeampipe2Length + 0.5*fBeampipe3FrontCoverThickness));
  ...
  new G4PVPlacement(0,
  		    G4ThreeVector(0,0,fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness */ + fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position + fWindowOuterJoint2_2dz /* + fWindowOuterJoint2_3Thickness */ + fBeampipe1Length + fBeampipe2Length - 0.5*fBeampipe3FrontCoverThickness),
  ...
  G4SubtractionSolid* sWindow_sub_InnerJoint2 = new G4SubtractionSolid("Window_sub_InnerJoint2",
  								       ...
  								       sWindowInnerJoint2,
  								       yRot,
  								       G4ThreeVector(fChamberOuterRadius + fWindowFrameThickness - fWindowThickness /*20180403*/ - 0.5*fWindowInnerJoint2Thickness, 0, 0));
  G4SubtractionSolid* sWindow_sub_InnerJoint2_sub_OuterJoint2 = new G4SubtractionSolid("Window_sub_InnerJoint2_sub_OutherJoint2",
  										       ...
  										       sWindowOuterJoint2,
  										       yRot,
  										       G4ThreeVector(fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* + fWindowOuterJoint1Thickness */ +0.5*fWindowOuterJoint2Thickness - fWindowOuterJoint2_1Position, 0, 0));
  ...
  G4SubtractionSolid* sWindow_sub_InnerJoint2_sub_OuterJoint2_sub_OuterJoint2_1 = new G4SubtractionSolid("Window_sub_InnerJoint2_sub_OutherJoint2",
													 ...
													 yRot,
													 G4ThreeVector(fChamberOuterRadius + fWindowFrameThickness /* + fWindowThickness 20180403*/ /* ????20180403 */ /*+ fWindowOuterJoint1Thickness */ + 0.5*fWindowOuterJoint2_1Thickness, 0, 0));
  //20180403(finish)

  fLogicChamberWindow = new G4LogicalVolume(sWindow_sub_InnerJoint2_sub_OuterJoint2_sub_OuterJoint2_1,//20181022, I think it was a mistake to use this -->//sWindow_sub_InnerJoint2_sub_OuterJoint2,
  					    fWindowMater,
  					    "Window_log");
  //
  ...
  G4SubtractionSolid* sChamberWindowVacuum_sub_InnerJoint2_sub_OuterJoint2_sub_InnerJoint1 = new G4SubtractionSolid("ChamberWindowVacuum_sub_InnerJoint2_sub_OutherJoint2_sub_InnerJoint1",
														    ...
														    sWindowInnerJoint1,
														    yRot,
														    G4ThreeVector(fChamberOuterRadius + fWindowFrameThickness - fWindowThickness /*20180403*/ -fWindowInnerJoint2Thickness - 0.5*fWindowInnerJoint1Thickness, 0, 0));
  ...
  //20190416(start)//HRS window installation
  //20190626(start)//Changed from HRS window(sphere) to HMS window(octagon)
  ...
  // 		    fCheckOverlaps);
  //Replicas
  // new G4PVReplica("Bulk_rep", lBulk, lNPS, kXAxis, 30, fSingle_X);//replicate each Bulks inside the NPS
  ...
  for(G4int ly = 0 ; ly < 30 ; ly++){
    for(G4int lx = 0 ; lx < 36 ; lx++){
      new G4PVPlacement(0,
			...
			lx + ly*36,
			fCheckOverlaps);
      // G4cout<<"x[cm] : "<<(14.5*fSingle_X - ly*fSingle_X)*1e-1<<",\t y[cm] : "<< (-17.5*fSingle_Y + lx*fSingle_Y)*1e-1<<G4endl;
    }
  }
  ...
  //
  // G4Box*
  //   sFluxOptFiber = new G4Box("FluxOptFiber_sol", 0.5*fWrapThickness, 0.5*fChamberHight, 0.5*1.5*m);

  // fLogicFluxOptFiber = new G4LogicalVolume(sFluxOptFiber,
  // 					   fWorldMater,
  // 					   "FluxOptFiber_log");
  // new G4PVPlacement(yMomRot,                     //no rotation
  // 		    G4ThreeVector((fMom_pos_Z + 0.5*fMom_Z + 0.5*1.5*m)*sin(-fMom_theta) - 0.5*fMom_X*cos(-fMom_theta), fMom_pos_Y, (fMom_pos_Z + 0.5*fMom_Z + 0.5*1.5*m)*cos(-fMom_theta) + 0.5*fMom_X*sin(-fMom_theta)),
  // 		    fLogicFluxOptFiber,
  // 		    "FluxOptFiber_pos",
  // 		    lWorld,
  // 		    false,
  ...
  //20190125(start)//Bigger FluxOptFiber to cover whole NPS

  //20190408(start)
  //These are all temporary
  //Going to use to compare the number of particles hitting the NPS and number of particles reconstructed
  // G4Box*
  //   sFluxOptFiber = new G4Box("FluxOptFiber_sol", 0.5*fWrapThickness, 0.5*fChamberHight, 0.5*(1.5*m + fMom_Z));

  // fLogicFluxOptFiber = new G4LogicalVolume(sFluxOptFiber,
  // 					   fWorldMater,
  // 					   "FluxOptFiber_log");
  // new G4PVPlacement(yMomRot,                     //no rotation
  // 		    G4ThreeVector((fMom_pos_Z + 0.5*(1.5*m + fMom_Z) - 0.5*fMom_Z)*sin(-fMom_theta) - 0.5*(fMom_X + fWrapThickness)*cos(-fMom_theta), fMom_pos_Y, (fMom_pos_Z + 0.5*(1.5*m + fMom_Z) - 0.5*fMom_Z)*cos(-fMom_theta) + 0.5*(fMom_X + fWrapThickness)*sin(-fMom_theta)),
  // 		    fLogicFluxOptFiber,
  // 		    "FluxOptFiber_pos",
  // 		    lWorld,
  // 		    false,
  ...
  //20171127(start)
  //
  //Pseudo volume to check the flux
  //
  G4double fFlux_Z = 65*1e-3*mm;
  G4Box*
  sFlux = new G4Box("Flux_sol", 0.5*fNPS_X, 0.5*fNPS_Y, 0.5*fFlux_Z);
  fLogicFlux = new G4LogicalVolume(sFlux,
  fWorldMater,//air
  "Flux_log");
  // new G4PVPlacement(0,
  // 		    G4ThreeVector(0, 0, -0.5*(fFlux_Z + fNPS_Z)),
  // 		    fLogicFlux,
  // 		    "Flux_pos",
  // 		    lTemp,
  // 		    false,
  ...
  */
  ...
  //(delete)
  /*
    G4double position1 = fSweepingMagnet_pos_Z/cos(fSweepingMagnet_theta);
    G4double position2 = fSweepingMagnet_X/2.;
    G4double position3 = position1*sin(fSweepingMagnet_theta);
    G4double position4 = (position2 + position3)*sin(fSweepingMagnet_theta);
    G4double position_X = (position2 + position3)*cos(fSweepingMagnet_theta);
    G4double position_Z = position1 - position4;
  */
  //20180412(finish)
  //20171130(start)
  // G4Box*
  //   sSweepingMagnetic = new G4Box("SweepingMagnetic_sol", 0.5*fSweepingMagnet_Mom_X, 0.5*fSweepingMagnet_Mom_Y, 0.5*fSweepingMagnet_Mom_Z);
  //20171130(finish)

  //20190624(start)
  //let's try subtracting the beam-pipe 1 itself!

  // //20181218(start)//subtract solid no to overlap with beam-pipe1. !!!!!!!!!!!1
  // G4Box*
  ...
  // G4Box* sSweepingMagnetic_sub_1 = new G4Box("SweepingMagnetic_sub_1", 0.5*(12.125 + 7.8 - 3.14404)*inch + 0.01*inch /*to make sure*//* + 0.00001*inch to make sure*/, 0.5*4.8*inch/* + 0.00001*inch to make sure*/, 0.5*(5.2 + 39.5)*inch/* + 0.00001*inch to make sure*/ );
  // G4Box* sSweepingMagnetic_sub_2 = new G4Box("SweepingMagnetic_sub_2", 0.5*(12.125 + 7.8 - 1.33307)*inch/* + 0.00001*inch to make sure*/, 0.5*2.4*inch/* + 0.00001*inch to make sure*/, 0.5*(5.2 + 4 + 3.2 + 3 + 1)*inch + 0.01*inch /*to make sure*/ );
  // G4Box* sSweepingMagnetic_sub_3 = new G4Box("SweepingMagnetic_sub_3", 0.5*7.5*inch, 0.5*60*inch, 0.5*5.2*inch /*+ 0.01*inch*/ /*to make sure*/ );

  // G4SubtractionSolid* sSweepingMagnetic_to_be_sub_1 = new G4SubtractionSolid("SweepingMagnetic_to_be_sub_1_sol",
  // 									     sSweepingMagnetic_to_be_sub,
  // 									     sSweepingMagnetic_sub_1,
  // 									     0,
  // 									     G4ThreeVector( -0.5*fSweepingMagnet_X + 0.5*(12.125 + 7.8 - 3.14404)*inch - 0.01*inch /*to make sure*/, 0, 0.5*fSweepingMagnet_Z - 0.5*(5.2 + 39.5)*inch ));

  // G4SubtractionSolid* sSweepingMagnetic_to_be_sub_2 = new G4SubtractionSolid("SweepingMagnetic_to_be_sub_2_sol",
  // 									     sSweepingMagnetic_to_be_sub_1,
  // 									     sSweepingMagnetic_sub_2,
  // 									     0,
  // 									     G4ThreeVector( -0.5*fSweepingMagnet_X + 0.5*(12.125 + 7.8 - 1.33307)*inch, 0, -0.5*fSweepingMagnet_Z + 0.5*(5.2 + 4 + 3.2 + 3 + 1)*inch + 0.01*inch /*to make sure*/ ));

  // G4SubtractionSolid* sSweepingMagnetic = new G4SubtractionSolid("SweepingMagnetic",
  // 								 sSweepingMagnetic_to_be_sub_1,
  // 								 sSweepingMagnetic_sub_2,
  // 								 0,
  // 								 G4ThreeVector( -0.5*fSweepingMagnet_X + 0.5*(12.125 + 7.8 - 1.33307)*inch, 0, -0.5*fSweepingMagnet_Z + 0.5*(5.2 + 4 + 3.2 + 3 + 1)*inch + 0.01*inch /*to make sure*/));
  ...
  G4SubtractionSolid* sSweepingMagnetic = new G4SubtractionSolid("SweepingMagnetic_sol",
  								 ...
  								 SM_BP_SubRot,
								 //20190905(start)
  								 // G4ThreeVector(x_global*cos(fSweepingMagnet_theta) + z_global*sin(fSweepingMagnet_theta), 0. - (fSweepingMagnetSTPcenter_Y - fSweepingMagnetSTPshift_Y) , - x_global*sin(fSweepingMagnet_theta) + z_global*cos(fSweepingMagnet_theta) ));//beam-pipe 1 global position - sweeping magnet global position
  ...
  // G4cout<<"fSweepingMagnet_theta : "<<fSweepingMagnet_theta*180./pi<<G4endl;
  ySweepingMagnetRot->rotateY(fSweepingMagnet_theta); //Angle in radian                     

  //20180928(start)
  //NEEDS G4PVPlacement to place according to the fSweepingMagnet_theta. //20181016. Done!    
  //20180928(finish)
  ...
  if(field){//20190110
    new G4PVPlacement(ySweepingMagnetRot,
		      //20180412(start)
		      //G4ThreeVector(position_X,0,position_Z),
		      //20190905(start)
		      // G4ThreeVector(fSweepingMagnet_pos*sin(-fSweepingMagnet_arm_theta), (fSweepingMagnetSTPcenter_Y - fSweepingMagnetSTPshift_Y), fSweepingMagnet_pos*cos(-fSweepingMagnet_arm_theta)),//20181016. changed from G4ThreeVector(fSweepingMagnetSTPcenter_X - fSweepingMagnetSTPshift_X, fSweepingMagnetSTPcenter_Y - fSweepingMagnetSTPshift_Y, fSweepingMagnetSTPcenter_Z - fSweepingMagnetSTPshift_Z),
  ...
  G4Box*
    //20190904(start)
    // sSweepingMagnet_1_4_1 = new G4Box("SweepingMagnet_1_4_1_sol", 0.5*5.35*inch, 0.5*30*inch, 0.5*39.5*inch);
  ...
  new G4PVPlacement(0,
		    //20190904(start)
		    // G4ThreeVector(4.225*inch + fSweepingMagnetShift_X, 0*inch + fSweepingMagnetShift_Y, 0 + fSweepingMagnetShift_Z),
  ...
  //20190904(start)
  // vertices_1_4_2.push_back(G4TwoVector(-0.5*7.48105*inch, -0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector(-0.5*7.48105*inch,  0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( 0.5*7.48105*inch,  0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( 0.5*7.48105*inch, -0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( (0.5*7.48105 - 0.10945)*inch, -0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( (0.5*7.48105 - 0.10945)*inch,  0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( 0.5*7.48105*inch,  0.5*14.0117*inch));
  // vertices_1_4_2.push_back(G4TwoVector( 0.5*7.48105*inch, -0.5*14.0117*inch));
  ...
  new G4PVPlacement(0,
		    //20190904(start)
		    // G4ThreeVector(-2.190525*inch + fSweepingMagnetShift_X, 0*inch + fSweepingMagnetShift_Y, 0 + fSweepingMagnetShift_Z),
		    // G4ThreeVector(-2.190525*inch + fSweepingMagnetShift_X + (5.35 - 4.6)*inch /* + 0.5*(7.5905 - 7.48105)*inch */, 0*inch + fSweepingMagnetShift_Y, 0 + fSweepingMagnetShift_Z),
		    //Relative porition from volume 1_4_1.
  ...
  return fPhysiWorld;
	...
	= new PMTcoverSD(SDname="/PMTcover");
      ...
      //to check background every where 4m from the target
	...
	= new ChambWindSD(SDname="/ChambWind");
      ...
      //20171010(start) can be used later for the magnetic field in certain volume
      // magnetic field ----------------------------------------------------------
      //
  ...
  /*
    ...
    localFieldMgr->SetDeltaIntersection(1e-4*m);
    localFieldMgr->SetDeltaOneStep(1e-4*m);
    logicEnv->SetFieldManager(localFieldMgr,true);
    G4cout << "Magnetic field has been constructed " << 
    "in DetectorConstruction::ConstructField()" << G4endl;
    fieldIsInitialized = true; 
    }
